...
export function RoundProvider(props: { children: JSX.Element }) {
  ...
  const syncClubs = async () => {
    try {
      ...
      // If we don't have headers (no user), checkUser will handle it eventually, 
      // but let's be safe
      if (Object.keys(headers).length === 0) return;

      const response = await fetch('/api/bag', { headers });
      if (response.ok) {
        const json = await response.json();
        // Check if data array is empty (no clubs)
        if (json.data && json.data.length === 0) {
          // No bag found, redirect to onboarding (which handles both user and bag now)
  ...
  const syncDown = async () => {
    try {
      const headers = await getAuthHeaders();
      const response = await fetch('/api/rounds', { headers });

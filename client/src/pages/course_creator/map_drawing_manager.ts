...
export class MapDrawingManager {
  ...
  // Track undo/redo stacks for drawing operations
  private undoStack: GeoJSON.FeatureCollection[] = [];
  private redoStack: GeoJSON.FeatureCollection[] = [];
  ...
  private handleDrawEvent(type: 'create' | 'update' | 'delete', e: any): void {
    ...
    if (type === 'create' || type === 'update' || type === 'delete') {
      setTimeout(() => this.saveDrawState(), 10);
    }
   ...
   */
  public undo(): void {
    if (!this.draw || this.undoStack.length === 0) return;
   ...
   */
  public redo(): void {
    if (!this.draw || this.redoStack.length === 0) return;
